...
const SKILLS: SkillSeed[] = [
...

  {
    ...
    body: [
      ...
      "- **Promoción** (`get_promos`): un descuento sobre esos planes, con vigencia y condiciones.",
      ...
      "3. `get_promos` → si ya hay una promo activa que se va a apilar con lo que estás por hacer.",
      "",
  ...
  {
    name: "estado-turistico",
    displayName: "Estado turístico de la zona",
    description:
      "Contexto turístico REAL de la zona de la propiedad (eventos cercanos, feriados y fines de semana largos, temporada, clima, " +
      "entorno a pie, interés online) leído de las fuentes del sistema. Si la charla necesita ese contexto y no está ya en el prompt, " +
      "usá `consultar_estado_turistico` en vez de buscar en la web. Cargala para saber cómo leer y contar esos datos.",
    tags: ["turismo", "demanda", "contexto", "base"],
    body: [
      "# Estado turístico de la zona",
      "",
      "## De dónde sale",
      "",
      "El sistema lee fuentes reales alrededor de la propiedad: agenda de eventos (Ticketmaster, Eventbrite,",
      "agendas oficiales) y grandes eventos de tablas curadas, vistas de Wikipedia del destino, feriados y",
      "recesos escolares del país y de los mercados emisores, clima promedio de 10 años, entorno a pie",
      "(OpenStreetMap) y alertas de desastres (GDACS). Los números los arma el código y se muestran en una",
      "tarjeta: vos no los redactás, los interpretás.",
      "",
      "## Cuándo usar la herramienta",
      "",
      "- Si el prompt ya trae el bloque \"Estado turístico de la zona\", NO la llames: ya lo tenés.",
      "- Si la conversación lo necesita (\"bloqueá el próximo finde largo\", \"¿conviene una promo para el",
      "  congreso?\"), llamá `consultar_estado_turistico` con la faceta justa: movimiento, eventos, entorno o",
      "  estacionalidad. Una o dos, no las cuatro.",
      "- La búsqueda web queda para algo puntual con nombre propio que las fuentes no cubren, y lo aclarás:",
      "  \"esto no es un dato verificado por el sistema\".",
      "",
      "## Cómo leerlo",
      "",
      "- [estimado] = tabla curada (mega-eventos, congresos, recesos aproximados): es una referencia general.",
      "- Un conteo con \"o más\" es un piso: la fuente recortó la lista.",
      "- \"Sin datos\" es que no se pudo leer. Nunca lo conviertas en cero ni lo completes con lo que sabés.",
      "- Cero eventos con ticket relevados NO es \"no hay eventos\": la cobertura depende de las ciudades que",
      "  sigue el radar.",
      "- El interés online son vistas de Wikipedia: anticipa la reserva por semanas, pero es un proxy.",
      "- El ruido del entorno es geometría (distancia a autopista, aeropuerto, bares), no una medición.",
      "- Los congresos de mitad de semana llenan martes a jueves; los festivales, el fin de semana.",
      "",
      "## Cómo contarlo",
      "",
      "- Principiante: qué significa para su alojamiento en palabras comunes, y una sola idea para aprovecharlo.",
      "- Avanzado: directo a la oportunidad (fechas con demanda, mercados que viajan, mínimos de estadía).",
      "- Si pregunta cómo aprovecharlo para vender más, ofrecé armar un plan: el plan es otra tarjeta.",
    ].join("\n"),
  },
];
