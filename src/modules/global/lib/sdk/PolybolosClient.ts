 ...
 * ║                                                                 ║
 * ║  Subscribes to elippser feeds + external providers (Lattice),     ║
...
// ── elippser Feed → Entity Translators ───────────────────────────────
...
function translateFlights(flights: any[], subtype: string): PolybolosEntity[] {
  ...
  return flights.map((f: any) => ({
    id: `elippser-air-${f.icao24 || f.callsign || Math.random().toString(36).slice(2)}`,
    ...
    source: { provider: 'elippser', feed: `flights-${subtype}`, originalId: f.icao24, confidence: 0.9 },
...
function translateMaritime(ships: any[]): PolybolosEntity[] {
  ...
  return ships.map((s: any) => ({
    id: `elippser-sea-${s.mmsi || s.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'elippser', feed: 'maritime-ais', originalId: s.mmsi?.toString(), confidence: 0.85 },
    timestamp: new Date().toISOString(),
...
function translateEarthquakes(events: any[]): PolybolosEntity[] {
  ...
  return events.map((eq: any) => ({
    id: `elippser-event-eq-${eq.id || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'elippser', feed: 'usgs-earthquakes', originalId: eq.id, confidence: 0.99 },
    timestamp: new Date().toISOString(),
...
function translateSatellites(sats: any[]): PolybolosEntity[] {
  ...
  return sats.map((s: any) => ({
    id: `elippser-space-${s.noradId || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'elippser', feed: 'satnogs', originalId: s.noradId?.toString(), confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
function translateFires(fires: any[]): PolybolosEntity[] {
  ...
  return fires.map((f: any, i: number) => ({
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'elippser', feed: 'nasa-firms', confidence: 0.9 },
    timestamp: new Date().toISOString(),
...
function translateCCTV(cameras: any[]): PolybolosEntity[] {
  ...
  return cameras.map((c: any) => ({
    id: `elippser-sensor-cctv-${c.id || Math.random().toString(36).slice(2)}`,
    name: c.name || 'Camera',
    domain: Domain.LAND,
    ...
    threat: ThreatLevel.NONE,
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'elippser', feed: 'cctv-network', originalId: c.id, confidence: 1.0 },
    timestamp: new Date().toISOString(),
...
function translateRadiation(stations: any[]): PolybolosEntity[] {
  ...
  return stations.map((r: any) => ({
    id: `elippser-sensor-rad-${r.name || Math.random().toString(36).slice(2)}`,
    ...
    classification: Classification.UNCLASSIFIED,
    source: { provider: 'elippser', feed: 'radiation-network', confidence: 0.95 },
    timestamp: new Date().toISOString(),
...
export class PolybolosClient {
  ...
  /** Connect to the elippser SSE stream endpoint */
  private connectSSE(): void {
    if (typeof EventSource === 'undefined') return;

    try {
      this.sseConnection = new EventSource(
        `${this.config.elippserBaseUrl}/api/sdk/stream`
      );
  ...
  /**
   * Ingest raw elippser data and translate it into Polybolos entities.
   * This is the primary method called by page.tsx to feed data into the SDK.
   */
  ingestElippserData(data: Record<string, any>): void {
  ...
  private getActiveFeedCount(): number {
    let count = 0;
    // Count elippser feeds that have data
