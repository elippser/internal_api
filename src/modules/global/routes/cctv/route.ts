...
/**
 * elippser — Worldwide CCTV Camera API v2
...
async function fetchTfLCameras(): Promise<any[]> {
  try {
    ...
    return (data || []).map((cam: any) => {
      const prop = (k: string) => cam.additionalProperties?.find((p: any) => p.key === k)?.value;
      const camId = cam.id?.replace('JamCams_', '') || '';
      // TfL publishes a rolling MP4 clip alongside the still — prefer the video.
      const videoUrl = prop('videoUrl') || `https://s3-eu-west-1.amazonaws.com/jamcams.tfl.gov.uk/${camId}.mp4`;
      return {
        id: `tfl-${cam.id}`, lat: cam.lat, lng: cam.lon,
        name: cam.commonName || 'London JamCam', city: 'London', country: 'UK',
        feed_url: prop('imageUrl') || `https://s3-eu-west-1.amazonaws.com/jamcams.tfl.gov.uk/${camId}.jpg`,
        stream_url: videoUrl,
        stream_type: 'mp4',
        source: 'TfL',
      };
    ...
    for (const result of results) {
      if (result.status === 'fulfilled') {
        for (const cam of result.value) {
          // Drop entries with nothing to render — a marker whose only link is an
          // external page is a dot that opens an empty viewer.
          if (!cam.feed_url && !cam.stream_url) continue;
          allCameras.push(cam);
