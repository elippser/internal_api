...
/**
 * elippser — Live Conflict Zone Intelligence API
 *
 * Aggregates conflict data from:
 * 1. World-news RSS feeds (BBC, Al Jazeera, NYT) — live headlines,
 *    keyword-matched to known conflict zones
 * 2. Curated active conflict zones — hand-maintained anchors with
 *    live event counts derived from the RSS matches
 *
 * NOTE: event coordinates are the matched zone's anchor (plus a small
 * cosmetic offset so dots don't stack) — they are zone-level, not the
 * precise location of the reported event.
 *
  ...
  try {
    ...
    const feedPromises = RSS_FEEDS.map(async (url) => {
      try {
        ...
        if (!res.ok) {
          console.log(`[elippser] RSS Fetch Failed for ${url}: ${res.status}`);
          return [];
        }
        
        const xml = await res.text();
        const rawItems = xml.split(/<item>/i).slice(1);
        console.log(`[elippser] RSS ${url} returned ${rawItems.length} items`);
        ...
        console.log(`[elippser] RSS Fetch Error for ${url}:`, (err as Error).message);
        return [];
      }
    });
    ...
    console.log(`[elippser] Mapped ${allEvents.length} conflict events from RSS.`);
  } catch (e) {
...
export async function GET() {
  try {
    ...
    console.error('[elippser] Conflict API error:', error);
