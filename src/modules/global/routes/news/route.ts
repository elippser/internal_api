...
/**
 * elippser — Military-Grade Intelligence API
...
// Curated defaults; override with ELIPPSER_TELEGRAM_CHANNELS=chan1,chan2,...
const TELEGRAM_CHANNELS = (process.env.ELIPPSER_TELEGRAM_CHANNELS || 'OSINTtechnical,Faytuks,Liveuamap,CyberKnow')
  .split(',')
  .map(c => c.trim())
  .filter(Boolean);
  ...
  try {
    ...
    const newsItems = allArticles.map(article => {
      ...
      return {
        ...
        // Coordinates come from a keyword→region lookup, not from the article itself
        coords_approx: !!coords,
