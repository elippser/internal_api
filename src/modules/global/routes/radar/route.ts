...
/**
 * elippser — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'elippser/4.2', 'Accept': 'application/json' },
    });

    console.log('[elippser] IODA response status:', res.status);
    ...
    console.error('[elippser] IODA fetch error:', error);
