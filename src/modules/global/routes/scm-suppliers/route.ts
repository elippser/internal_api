...
/**
 * elippser — SCM Supplier Risk Overlay
...
export async function GET(request: Request) {
  // Derive our own origin from the incoming request — a hardcoded
  // http://127.0.0.1:3000 breaks under Docker or any non-default port.
  ...
  try {
    ...
    // 2. Fetch Active Fires — reuse the local fires endpoint since it already aggregates NASA FIRMS
    ...
    // 3. Fetch disaster/conflict events (GDACS via the local /api/gdelt route)
