...
/**
 * elippser — Maritime Intelligence
...
function connectAisStream() {
  ...
  // Map AIS ship types to elippser categories
  const getElippserShipType = (typeCode: number) => {
  ...
  ws.on("message", (data) => {
    try {
      ...
      if (parsed.MessageType === "PositionReport" && parsed.Message?.PositionReport) {
        ...
        existing.type = getElippserShipType(staticData.Type);
      }
