...
// Country Intelligence Index — curated (editorial) baseline scores,
// enriched with live USGS earthquake activity. The base numbers are a
// hand-maintained index, not a live feed.
  ...
  try {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Match USGS place strings against real country names — matching the
        // 2-letter ISO code as a substring produced false hits ("IR" in "Kiribati")
        const COUNTRY_NAMES: Record<string, string[]> = {
          UA: ['ukraine'], RU: ['russia'], IL: ['israel'], PS: ['palestin', 'gaza strip', 'west bank'],
          SY: ['syria'], YE: ['yemen'], MM: ['myanmar', 'burma'], SD: ['sudan'],
          AF: ['afghanistan'], KP: ['north korea'], IR: ['iran'], CN: ['china'],
          TW: ['taiwan'], VE: ['venezuela'], HT: ['haiti'], LB: ['lebanon'],
          PK: ['pakistan'], SO: ['somalia'], LY: ['libya'], ET: ['ethiopia'],
        };
        for (const f of data.features || []) {
          const place = (f.properties?.place || '').toLowerCase();
          const mag = f.properties?.mag || 0;
          for (const [code, names] of Object.entries(COUNTRY_NAMES)) {
            if (names.some(n => place.includes(n))) {
              quakeRisks[code] = (quakeRisks[code] || 0) + mag;
            }
          }
        }
      }
    } catch (e) { console.warn('[elippser] Suppressed error:', e instanceof Error ? e.message : e); }
