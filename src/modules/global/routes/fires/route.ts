...
/**
 * elippser — Active Fire & Wildfire Tracking
...
export async function GET() {
  try {
    ...
    for (const url of firmsSources) {
      try {
        const res = await fetch(url, {
          ...
          headers: { 'User-Agent': 'elippser-Intelligence-Platform/3.5' },
        });
        if (res.ok) {
      ...
      if (volcRes.ok) {
        ...
        const volcanoes = (volcData.events || []).map((e: any) => {
          ...
          return {
            ...
            // EONET reports volcanic events, not thermal readings — no
            // brightness/FRP values exist for these, so none are invented.
            brightness: null,
            confidence: 'reported',
    ...
    } catch (e) { console.warn('[elippser] Suppressed EONET error:', e instanceof Error ? e.message : e); }
